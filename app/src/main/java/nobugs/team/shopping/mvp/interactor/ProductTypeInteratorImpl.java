...
public class ProductTypeInteratorImpl implements ProductTypeInterator {
    ...
    private ArrayList<ProductTypePo> mProductTypePos;
    ...
    private void loadProductTypeFromJson(String jsonTest) {
        Gson gson = new Gson();
        mProductTypePos = gson.fromJson(jsonTest, new TypeToken<ArrayList<ProductTypePo>>(){}.getType());
    }

    private List<ProductTypePo> findProductTypeByParentId(int parentid) {
        List<ProductTypePo> result = new ArrayList<>();
        if (mProductTypePos != null) {
            for (int i = 0; i < mProductTypePos.size(); i++) {
                if (mProductTypePos.get(i).getParentid().equals(parentid)) {
                    result.add(mProductTypePos.get(i));
                }
            }
        }
        return result;
